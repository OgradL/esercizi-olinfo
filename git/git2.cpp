...
void fix_lazy(Node* node, int l, int r){
	...
	if (r - l > 1){
		// Node *prevr = node->right, *prevl = node->left;
		// node->left = new Node();
		// node->right = new Node();

		// node->left->left = prevl->left;
		// node->left->right = prevl->right;
		// node->left->sum = prevl->sum;
		node->left->lazyAdd += /*prevl->lazyAdd*/ + node->lazyAdd;

		// node->right->left = prevr->left;
		// node->right->right = prevr->right;
		// node->right->sum = prevr->sum;
		node->right->lazyAdd += /*prevr->lazyAdd*/ + node->lazyAdd;
	}
	node->lazyAdd = 0;
}
